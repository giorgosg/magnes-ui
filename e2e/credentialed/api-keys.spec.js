// API-key management, driven end to end: make a key, read its secret the one time it
// exists, and take it back. Every screen in this feature that got a real browser pass found
// a defect every unit test had passed through, and this screen — a secret shown once, a
...
test.describe("API-key management", () => {
  ...
  test("makes a key, shows its secret once, and lists it afterwards", async ({ page }) => {
    ...
    // The secret exists in this render and nowhere afterwards.
    const reveal = page.getByRole("alert").filter({ hasText: "only time it is shown" });
    await expect(reveal).toBeVisible();
    const secret = await reveal.locator("code").innerText();
    expect(secret.length).toBeGreaterThan(0);

    await page.getByRole("button", { name: "I have stored it" }).click();

    // Dismissed, the secret is gone and the key is in the list, read back from the server.
  ...
  test("the secret is not offered as retrievable later", async ({ page }) => {
    ...
    // Reloading the page re-fetches the listing. Nothing in it is the secret: the API has no
    // field that returns one, so a screen that showed a key's secret here would be inventing it.
    await page.reload();
    const row = page.getByRole("listitem").filter({ hasText: name });
    await expect(row).toBeVisible();
