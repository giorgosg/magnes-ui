...
At runtime the browser does not talk to bitmagnet directly: requests go to the Magnes
server (SQLite-backed users, optional guest access), which proxies to bitmagnet. So the
GraphQL endpoint in Elm is a same-origin path like `/graphql`, not a bitmagnet host —
no CORS, no absolute URL, and auth rides along as a normal cookie/header. Code generation
still points at bitmagnet directly, since the schema is identical either way.
