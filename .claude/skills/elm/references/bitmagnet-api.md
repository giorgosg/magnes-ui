...
[verified] Both the POST and the preflight return `Access-Control-Allow-Origin: *`
(`Access-Control-Allow-Methods: POST`, `Access-Control-Allow-Headers: content-type`). A
browser can query bitmagnet directly with no proxy in the path — which is what makes the
serverless phase-1 UI possible, and equally what makes the proxy worth building later.
...
`delete` permanently removes torrents from the index. This is the concrete reason the proxy
exists and why guests must never reach `Mutation` — an unauthenticated visitor with direct
API access could empty the index.

Suggested proxy allowlist per guest level:

| Level | Allowed |
| --- | --- |
| none | — |
| search | `torrentContent.search`, `torrent.files`, `torrent.listSources` |
| search + dashboard | the above plus `torrent.metrics`, `queue.metrics`, `health`, `version` |
| (any guest level) | **never** `Mutation`, never `queue` mutations |
